...
Usage:
    # Analyze designs from CSV with default RPM (3000)
    python analysis.py input.csv output.csv
    
    # Analyze with custom RPM
    python analysis.py input.csv output.csv --rpm 4000
...
def format_results(design: WingDesign, conditions: OperatingConditions, 
    ...
    lines.append(f"  Thrust (Lift):        {results['thrust_total']*1e6:8.2f} μN")
    lines.append(f"  Power Required:       {results['power_total']*1e6:8.2f} μW")
    lines.append(f"  Torque:               {results['torque_total']*1e9:8.3f} μN·mm")
    ...
    try:
        with open(output_file, 'w', newline='') as f:
            fieldnames = [
                ...
                'rpm', 'rho', 'altitude',
                'thrust_uN', 'power_uW', 'torque_uNmm',
            ...
            for design, results in zip(designs, all_results):
                row = {
                    ...
                    'thrust_uN': results['thrust_total'] * 1e6,
                    'power_uW': results['power_total'] * 1e6,
                    'torque_uNmm': results['torque_total'] * 1e9,
                    'ct': results['ct'],
                    'cp': results['cp'],
...
Examples:
  ...
  # Adjust for altitude
  python analysis.py input.csv output.csv --altitude 1000
        """
    )
    
    # Positional arguments (required)
    parser.add_argument('csv', type=str, help='CSV file with wing parameters')
    ...
    parser.add_argument('--rpm', type=float, default=3000.0,
                       help='Rotation speed in RPM (default: 3000)')
    ...
    # Write results to CSV (required output)
    ...
    if not args.quiet and len(analyzed_designs) > 1:
        ...
        print(f"Average Thrust: {avg_thrust*1e6:.2f} μN")
        print(f"Average Power:  {avg_power*1e6:.2f} μW")
        ...
        print(f"  Thrust = {all_results[best_idx]['thrust_total']*1e6:.2f} μN")
        print(f"  Power = {all_results[best_idx]['power_total']*1e6:.2f} μW")
